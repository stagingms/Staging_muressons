...
| Share count: 6,500,000 in code vs 100,000,000 in the book | **Listed here in error.** Closed on 4 September, before Phase 5: Ch 14 §14.5.2, Ch 18 §18.2.8 and both Student Manuals all read 6,500,000, and each records the change. Ch 14's own editorial note says "the second (share count) was closed on 4 September." **One straggler found 8 September**, when this row was re-checked a second time: Ch 14's §14.4 worked example still printed "Price per share. $7.56" against $756.2m of equity — the ÷100,000,000 figure. §14.5.2's formula and the Meridian/Aurora examples were already correct, which is why the first re-check passed. Now $116.34. | CLOSED (one worked example corrected 8 September) |
...
| Published M_R ceilings quoted as 1.93 / 2.02 / 1.98 | §4.2 — quoted as maxima when they are base ceilings indexed on the paradigm, which is not the binding dimension | **FIXED 8 September** — Ch 14 §14.3.2 and its worked example, and Ch 19 §19.6, now state that four of five pathways add up to +0.65 outside the component sum and are bound by the 2.05 clamp |
| Supply-chain transparency described as a moving score | §4.1 — constant at 40.0 for every session graded under 2026.09 | **FIXED 8 September** — Ch 6 §6.3's worked chain (a Round 1 deep audit "adds 20", cutting the nature premium to 0.54%) now records that the lever was inert until then; and the five audit findings in Book Part 4/5 (¶150, ¶816, the Part 4 Case 3 correction table) and Appendices A/B (§B.4's effect table, the Part-4-vs-engine table) now carry the revival. Part 4 ¶816's facilitator instruction — "do not let a student cost the exit against a penalty that never lands" — is explicitly reversed for 2026.10 cohorts, where the −10 does land |
| Any passage quoting an M_R efficiency percentage | §4.2 — the denominator changed | **NONE FOUND** — swept all 47 manuscript documents; the three matches are "water_efficiency_all" and the circular-economy efficiency bonus. Withdrawn |
