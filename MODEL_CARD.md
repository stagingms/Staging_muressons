...
| Share count / reveal price (`TV_SHARES_OUTSTANDING`, changed 2026-09-04, F-20) | 6,500,000 shares at a $50 IPO = baseline EV ($19.2M seed EBITDA × 17) ÷ $50, so the reveal's price per share is on the scale of the terminal EVs the model produces: a $280M EV prices at ≈ $43 (amber), $891M at ≈ $137 (green). Was 100,000,000 — a $5.0B implied market cap against $0–0.9B EVs, so every solvent team read "$1–3, loss −94%" in red. The two client-side copies (`stockValuationEngine.js`, `TerminalValuationCalc.js`) mirror the config value; the volume patcher carries it to production. | A share count is a presentation choice — no benchmark | `stylized` — chosen so the IPO anchor is the baseline company, which is what the ticker and the colour ladder assume |
| NCD liability in the reveal / solvency gate (changed 2026-09-04, F-20) | NCD is an index (hard cap 5,000; R10 group sums 47–92 observed). The reveal's "Adjusted Value = treasury × M_R − NCD" and the archetype solvency gate subtracted the raw index from dollars (inert: "$61.1M − $47"). Now the liability is the engine's own price of natural-capital debt capitalised as the EV capitalises EBITDA: `points × NCD_OPEX_PENALTY_PER_UNIT ($1,000/pt/round) × exit multiple` — e.g. 47 pts × $1,000 × 12 ≈ $0.56M; a neglected group at the 5,000 cap per BU ≈ $60M × BUs. Published as `ncd_liability_usd` with `total_ncd_points` and a `basis` string; the reveal shows both. | Natural-capital liabilities are not market-priced | `pedagogical` — the capitalisation of a recurring penalty is the one reading of "liability" the model can defend from its own constants; the magnitude inherits the NCD OPEX row's verdict |
