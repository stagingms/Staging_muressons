...
function NegotiationTranscripts({ sessionId }) {
    ...
    return (
        <div style={{ marginTop: '0.75rem', padding: '10px 12px', borderRadius: 10, background: 'rgba(148,163,184,0.06)', border: '1px solid rgba(148,163,184,0.18)' }}>
            <div style={{ fontSize: 'var(--type-caption)', fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>
                🤝 Negotiation transcripts ({data.rooms.length})
            </div>
            {data.rooms.map((room, i) => (
                <div key={i} style={{ marginBottom: 6 }}>
                    <button type="button" onClick={() => setOpenIdx(openIdx === i ? null : i)}
                        style={{ width: '100%', textAlign: 'left', display: 'flex', justifyContent: 'space-between', gap: 8, padding: '6px 10px', borderRadius: 8, cursor: 'pointer', background: 'rgba(148,163,184,0.08)', border: '1px solid rgba(148,163,184,0.15)', color: 'var(--text-primary)', fontSize: '0.76rem' }}>
                    ...
                    {openIdx === i && (
                        <div style={{ padding: '8px 10px', fontSize: 'var(--type-caption)', lineHeight: 1.5 }}>
                            {(room.turns || []).map((t, j) => (
                                <div key={j} style={{ marginBottom: 4, color: t.who === 'player' ? 'var(--text-primary)' : 'var(--text-secondary)' }}>
            ...
            {data.promises?.length > 0 && (
                <div style={{ marginTop: 4, fontSize: 'var(--type-caption)', color: 'var(--text-secondary)' }}>
