...
export default function RegretMeter({ mr = 0, flags = {} }) {
  ...
  if (mrLeftOnTable <= 0) {
    return (
      <div style={wrap}>
        <div style={{ ...headline, color: 'var(--kpi-good)' }}>
  ...
  return (
    <div style={wrap}>
      <div style={headline}>
        You left <strong style={{ color: 'var(--kpi-warn)' }}>{mrLeftOnTable.toFixed(2)} M_R</strong> on the table.
      </div>
      ...
      <div style={{ position: 'relative', height: 14, borderRadius: 7, background: 'rgba(148,163,184,0.15)', overflow: 'hidden', marginBottom: 12 }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: `${pct}%`, background: 'linear-gradient(90deg, var(--accent-blue), var(--kpi-good))', borderRadius: 7, transition: 'width 0.8s ease' }} />
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {missed.map((m) => (
          <div key={m.flag} style={row}>
            <span style={{ color: 'var(--kpi-warn)' }}>✗ R{m.round} · {m.label}</span>
            <span style={{ fontWeight: 700, color: 'var(--kpi-warn)' }}>+{m.mr.toFixed(2)} missed</span>
          </div>
        ))}
        {captured.map((m) => (
          <div key={m.flag} style={{ ...row, opacity: 0.75 }}>
            <span style={{ color: 'var(--kpi-good)' }}>✓ R{m.round} · {m.label}</span>
            <span style={{ fontWeight: 700, color: 'var(--kpi-good)' }}>+{m.mr.toFixed(2)} captured</span>
          </div>
        ))}
      </div>
    </div>
...
const headline = { fontSize: '1.05rem', fontWeight: 700, marginBottom: 4, color: 'var(--text-primary)' };
