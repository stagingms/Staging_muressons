...
export default function ArchetypeCard({
  ...
  const download = useCallback(() => {
    ...
    const price = priceNum != null ? `${currencySymbol()}${atRate(priceNum).toFixed(2)}` : '—';
    const svg = `
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <g font-family="'DM Sans',Arial,sans-serif" text-anchor="middle">
    ...
    <text x="600" y="500" font-size="52" font-weight="800" fill="#f1f5f9">${currencySymbol()}${atRate(Number(terminalValueM)).toFixed(1)}M</text>
