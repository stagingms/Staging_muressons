...
export default function ArchetypeCard({
  ...
  const download = useCallback(() => {
    ...
    const price = sharePrice != null ? `$${Number(sharePrice).toFixed(2)}` : '—';
    const svg = `
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <rect x="8" y="8" width="${W - 16}" height="${H - 16}" rx="24" fill="none" stroke="${esc(accent)}" stroke-opacity="0.35" stroke-width="2"/>
  ...
  <text x="600" y="250" text-anchor="middle" font-size="120">${esc(icon)}</text>
  <text x="600" y="340" text-anchor="middle" font-family="'DM Sans',Arial,sans-serif" font-weight="800" font-size="64" fill="#f1f5f9">${esc(title)}</text>
  ${cohortName ? `<text x="600" y="384" text-anchor="middle" font-family="'DM Sans',Arial,sans-serif" font-size="26" fill="#b0bec5">${esc(cohortName)}</text>` : ''}
  <g font-family="'DM Sans',Arial,sans-serif" text-anchor="middle">
    <text x="300" y="500" font-size="52" font-weight="800" fill="${esc(accent)}">${Number(mr).toFixed(2)}×</text>
    ...
    <text x="900" y="500" font-size="52" font-weight="800" fill="#f1f5f9">${esc(price)}</text>
    ...
    img.onload = () => {
      ...
      canvas.toBlob((png) => {
        ...
        a.download = `muressons-${title.toLowerCase().replace(/[^a-z0-9]+/g, '-')}.png`;
    ...
    img.src = url;
  }, [title, icon, mr, terminalValueM, sharePrice, accent, cohortName]);
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `1px solid ${accent}`, background: `${accent}22`, color: 'var(--text-primary, #f1f5f9)',
