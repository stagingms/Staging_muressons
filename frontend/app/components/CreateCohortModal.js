            ...
            <div className={styles.modal} style={{ position: 'relative' }}>
                ...
                {setupResults && (
                    <div style={{
                        ...
                        {/* RBAC-F6: a project_admin provisions the cohort SHELL; player ids are
                            live-run credentials, so require_sim_manager (which excludes
                            project_admin) mints them. Say so HERE — previously the only signal
                            was a 403 after they had already gone looking for the roster. */}
                        {isProjectAdmin && (
                            <div style={{
                                margin: '0 0 1rem', padding: '0.7rem 0.9rem', borderRadius: 8,
                                background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.28)',
                                color: '#93c5fd', fontSize: '0.76rem', lineHeight: 1.5,
                            }}>
                                <strong style={{ color: '#bfdbfe' }}>Next step is the facilitator&rsquo;s.</strong>{' '}
                                Provisioning is complete — but player IDs are generated by the
                                facilitator who runs this cohort, not by a project administrator.
                                Hand it over and they will create and distribute the roster.
                            </div>
                        )}
