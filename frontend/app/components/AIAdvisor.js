    ...
    if (q.includes('compare') || q.includes('option') || q.includes('vs')) {
      ...
      opts.forEach(([key, opt]) => {
        ...
        if (opt.impacts?.treasury || opt.cost_impact) {
          response += `Cost Impact: ${currencySymbol()}${atRate((opt.impacts?.treasury || opt.cost_impact) / 1_000_000).toFixed(1)}M\n`;
        }
        response += '\n';
      });
      response += `**My Recommendation:** `;
      if (treasury < 35_000_000) response += `Given your low treasury (${currencySymbol()}${atRate(treasury / 1_000_000).toFixed(1)}M), consider a cost-conservative option.`;
