...
function BarChart3({ cogs, profit, revenue }) {
    ...
    const fmtK = v => `${currencySymbol()}${atRate(v / 1000).toFixed(0)}k`;
    return (
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxHeight: H }}>
            {[0, 0.25, 0.5, 0.75, 1].map((f, i) => {
                ...
                return <g key={i}>
                    ...
                    <text x={padL - 3} y={y + 3} textAnchor="end" fontSize="8" fill="#cbd5e1">{`${currencySymbol()}${atRate((f * maxV) / 1000).toFixed(0)}k`}</text>
                </g>;
            })}
                ...
                <div style={{ background: '#fff', padding: '0.9rem 1.5rem', borderBottom: '1px solid #e2e8f0' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                        ...
                        <div style={{ display: 'flex', gap: '1.2rem', fontSize: 'var(--type-caption)' }}>
                            {[['CO2 INTENSITY', `${avgCO2.toFixed(2)} kg/u`], ['UNIT COST', `${currencySymbol()}${atRate(avgCost).toFixed(2)}`], ['GROSS PROFIT', `${currencySymbol()}${atRate(profit / 1000).toFixed(0)}k`], ['STATUS', contractSafe ? 'SECURED' : 'LOST']].map(([k, v]) => (
                    ...
                    {SUPPLIERS.map((s, i) => (
                        <div key={s.id} style={{ gridColumn: i === 2 ? '1' : 'auto' }}>
                            <div style={{ fontSize: 'var(--type-caption)', color: '#475569', fontWeight: 600, marginBottom: '3px', display: 'flex', justifyContent: 'space-between' }}>
                                <span>{s.label}</span>
                                <span style={{ color: s.color }}>CO₂: {s.co2}kg · {currencySymbol()}{atRate(s.cost)}/u</span>
                            </div>
