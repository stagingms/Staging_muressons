...
$$\\text{M_R Bonus: }+0.10 \\text{ iff WRI} \\geq 75 \\text{ at R10}$$`,
      ...
      },
      {
        ...
        definition: 'An automated detection engine that identifies discrepancies between stated ESG commitments and actual investment behaviour. Every option carries a green-claim level (full / moderate / none). A full claim backed by an average investment ratio below 15% of the CSF pool — or a moderate claim below ≈10% — triggers a 15-point (moderate: 7.5-point) SLO penalty across ALL business units, unless average CapEx is at least $3M per BU. A scandal is also remembered as a betrayal by the stakeholder agents.',
        logic: 'Computed in engine.py → calc_greenwashing_risk(). Reads the chosen option\'s green_claim (full 15% bar, moderate ≈10% bar, none → not checked), the pre-austerity average investment ratio and the $3M/BU absolute escape. The penalty is applied to social_license_score for every BU simultaneously, modelling the reputational cascade from a public greenwashing scandal.',
        formula: `$$\\text{If claim = full AND }\\frac{1}{n}\\sum_{i=1}^{n} \\text{InvRatio}_i < 0.15 \\text{ (moderate: } < 0.10\\text{) AND avg CAPEX} < \\$3M: \\quad \\forall i:\; \\text{SLO}_i \\mathrel{-}= 15 \\text{ (moderate: } 7.5)$$`,
